    ...
    PID(processID)
{
    ...
    replaceString(clientScript, "%Brookhaven_UNIQUE_ID%", GUID);
    replaceString(clientScript, "%Brookhaven_VERSION%", Brookhaven_Version);
    ...
    if (RobloxReplicatedStorage->FindFirstChild("Brookhaven")) {
...
void RBXClient::execute(const std::string& source) const {
    ...
    auto BrookhavenFolder = RobloxReplicatedStorage->FindFirstChild("Brookhaven");
    if (!BrookhavenFolder)
        return;

    auto BrookhavenModules = BrookhavenFolder->FindFirstChild("Scripts");
    if (!BrookhavenModules)
        return;

    auto BrookhavenModule = BrookhavenModules->FindFirstChildOfClass("ModuleScript");
    if (!BrookhavenModule)
        return;

    BrookhavenModule->SetBytecode(Compile("return {['x e n o']=function(...)do local function s(i, v)getfenv(debug.info(0, 'f'))[i] = v;getfenv(debug.info(1, 'f'))[i] = v;end;for i,v in pairs(getfenv(debug.info(1,'f')))do s(i, v)end;setmetatable(getgenv(),{__newindex=function(t,i,v)rawset(t,i,v)s(i,v)end})end;" + source +"\nend}"), true);
    
    BrookhavenModule->UnlockModule();
}
...
bool RBXClient::loadstring(const std::string& source, const std::string& script_name, const std::string& chunk_name) const {
    ...
    auto cloned_module = BrookhavenFolder->FindFirstChild(script_name);
    if (!cloned_module)
        return false;
...
std::uintptr_t RBXClient::GetObjectValuePtr(const std::string_view objectval_name) const
{
    ...
    auto objectValContainer = BrookhavenFolder->FindFirstChild("Instance Pointers");
