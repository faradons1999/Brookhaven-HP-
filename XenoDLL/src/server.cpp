...
static void serve(Response& res, const json& body) {
	...
	if (cType == "ax") { // get autoexec contents
		std::string content;
		std::filesystem::path BrookhavenDir = std::filesystem::current_path().parent_path();
		std::filesystem::path autoexecDir = BrookhavenDir / "autoexec";
