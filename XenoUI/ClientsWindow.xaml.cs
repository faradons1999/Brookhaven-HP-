...
namespace BrookhavenUI
{
	public partial class ClientsWindow : Window
	{
		public string BrookhavenVersion = "1.0.7";
		...
		[DllImport("BrookhavenDLL.dll", CallingConvention = CallingConvention.Cdecl)]
		private static extern void Initialize();

		[DllImport("BrookhavenDLL.dll", CallingConvention = CallingConvention.Cdecl)]
		private static extern IntPtr GetClients();

		[DllImport("BrookhavenDLL.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
		...
		private async void LoadSupportedVersion()
		{
			try
			{
				...
				string latestVersion = await client.GetStringAsync("https://rizve.us.to/Brookhaven/LatestVersion");
				if (latestVersion != BrookhavenVersion)
				{
					MessageBox.Show($"The current version {BrookhavenVersion} is outdated.\n\nPlease download the latest version of Zenora ({latestVersion}) here: https://discord.gg/exploitnews", "Outdated Zenora version", MessageBoxButton.OK, MessageBoxImage.Warning);
					Application.Current.Shutdown();
				}
				SupportedVersion = await client.GetStringAsync("https://rizve.us.to/Brookhaven/SupportedVersion");
			}
